...
namespace BufLib.Common.Compression.Nintendo
{
    ...
    {
        //Huffman 4bit/8bit
        public static byte[] Decompress(Stream input, int num_bits, int decompressedLength, Endian byteOrder = Endian.LittleEndian)
        {
            using (var br = new BinaryReader(input))
            {
                var result = new List<byte>(decompressedLength);
                ...
                for (int i = 0, code = 0, next = 0, pos = tree_root; ; i++)
                {
                    ...
                    if (result.Count == decompressedLength * 8 / num_bits)
                    {
                        ...
                        {
                            if (byteOrder == Endian.LittleEndian)
                            {
        ...
        public static byte[] Compress(Stream input, int num_bits, Endian byteOrder = Endian.LittleEndian)
        {
