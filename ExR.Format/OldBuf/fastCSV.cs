...
using System.IO;
//using System.Reflection.Emit; // DC_FIX

internal class fastCSV
{
...
//#endif
    ...
    {
        COLUMNS.MGSpan[] cols;
        //List<T> list = new List<T>(10000); // DC_FIX
        List<T> list = null; // DC_FIX

        int linenum = 0;
        // CreateObject co = FastCreateInstance<T>(); // DC_FIX
        ...
        T df = default; // DC_FIX, TOOD: null line support?
        while (true)
        {
            try
            {
                ...
                //if (b)
                //    list.Add(o);
                _ = mapper(df, new COLUMNS(c, names)); // DC_FIX
            }
            catch (Exception ex)
            {
                sr.Close();
    ...
    public static void WriteStream<T>(Stream f, string[] headers, char delimiter, List<T> list, FromObj<T> mapper)
    {
        using (StreamWriter s = new StreamWriter(f))
        {
            ...
            foreach (var o in list)
            {
                ...
                for (int i = 0; i < cols.Count; i++)
                {
                    ...
                    var str = cols[i]?.ToString() ?? string.Empty;
                    bool quote = false;
    ...
    public static void WriteFile<T>(string filename, string[] headers, char delimiter, List<T> list, FromObj<T> mapper)
    {
        ...
        {
            WriteStream(f, headers, delimiter, list, mapper);
        }
    }

    //private delegate object CreateObject(); // DC_FIX
