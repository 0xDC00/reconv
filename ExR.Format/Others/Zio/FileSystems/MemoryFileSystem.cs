...
namespace Zio.FileSystems
{
    ...
    public class MemoryFileSystem : FileSystem
    {
        ...
        //private readonly object _dispatcherLock; // DC_FIX
        //private FileSystemEventDispatcher<Watcher>? _dispatcher; // DC_FIX
        ...
        public MemoryFileSystem()
        {
            ...
            //_dispatcherLock = new object(); // DC_FIX
        }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                //TryGetDispatcher()?.Dispose(); // DC_FIX
            }
        }
        ...
        protected override void CreateDirectoryImpl(UPath path)
        {
            ...
            try
            {
                ...
                //TryGetDispatcher()?.RaiseCreated(path); // DC_FIX
            }
            finally
            {
            ...
            try
            {
                ...
                finally
                {
                    if (deleteRootDirectory)
                    {
                        ...
                        //TryGetDispatcher()?.RaiseDeleted(path); // DC_FIX
                    }
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (destNode is null)
                        {
                            ...
                            //TryGetDispatcher()?.RaiseCreated(destPath); // DC_FIX
                            //TryGetDispatcher()?.RaiseChange(destPath); // DC_FIX
                        }
                        else if (overwrite)
                        {
                ...
                try
                {
                    ...
                    if (!destBackupPath.IsNull)
                    {
                        ...
                        if (backupResult.Node != null)
                        {
                            ...
                            //TryGetDispatcher()?.RaiseDeleted(destBackupPath); // DC_FIX
                        }

                        destResult.Node.DetachFromParent();
                        destResult.Node.AttachToParent(backupResult.Directory!, backupResult.Name!);

                        //TryGetDispatcher()?.RaiseRenamed(destBackupPath, destPath); // DC_FIX
                    }
                    else
                    {
                        destResult.Node.DetachFromParent();
                        ...
                        //TryGetDispatcher()?.RaiseDeleted(destPath); // DC_FIX
                    }

                    srcResult.Node.DetachFromParent();
                    srcResult.Node.AttachToParent(destResult.Directory!, destResult.Name!);

                    //TryGetDispatcher()?.RaiseRenamed(destPath, srcPath); // DC_FIX
                }
                finally
                {
        ...
        protected override void SetAttributesImpl(UPath path, FileAttributes attributes)
        {
            ...
            //TryGetDispatcher()?.RaiseChange(path); // DC_FIX
        }
        ...
        private void MoveFileOrDirectory(UPath srcPath, UPath destPath, bool expectDirectory)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // TryGetDispatcher()?.RaiseDeleted(srcPath); // DC_FIX
                    //TryGetDispatcher()?.RaiseCreated(destPath); // DC_FIX
                }
                finally
                {
        ...
        private class DirectoryNode : FileSystemNode
        {
            ...
            {
                _children = new Dictionary<string, FileSystemNode>(StringComparer.OrdinalIgnoreCase); // DC_FIX (case)
            }
            ...
            public override FileSystemNode Clone(DirectoryNode? newParent, string? newName)
            {
                ...
                dir._children = new Dictionary<string, FileSystemNode>(StringComparer.OrdinalIgnoreCase);  // DC_FIX (case)
